    ...
    if(this != NULL && pFunc != NULL && (order == 1 || order == 0))
    {
        ...
        for(i = 0; i < ll_len(this) - 1; i++)
        {
            ...
            for(j = i + 1; j < ll_len(this); j++)
            {
                ...
                if(pFunc(pNodoUno->pElement, pNodoDos->pElement) == 1)   //La funcion criterio debe devolver [1] si el primer elemento es mayor al segundo,
                {                                                        // [-1] si el primer elemento es menor al segundo, y [0] si son iguales
                    if(order == 1)
                    {
                        pAuxiliar = pNodoDos->pElement;
                        pNodoDos->pElement = pNodoUno->pElement;
                        pNodoUno->pElement = pAuxiliar;
                    }
                }

                if(pFunc(pNodoUno->pElement, pNodoDos->pElement) == -1)
                {
                    if(order == 0)
                    {
                        pAuxiliar = pNodoDos->pElement;
